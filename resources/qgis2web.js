...
map
  .getView()
  .fit(
    [-7600051.820774, -2126429.503812, -7004562.748886, -1761401.178075],
    map.getSize()
  );
    ...
    if (
      ...
      if (
        ...
        if (/\.(gif|jpg|jpeg|tif|tiff|png|avif|webp|svg)$/i.test(fieldValue)) {
          ...
          popupField +=
            fieldValue != null
              ? '<audio controls><source src="images/' +
                fieldValue.replace(/[\\\/:]/g, "_").trim() +
                '" type="audio/mpeg">Il tuo browser non supporta il tag audio.</audio></td>'
              : "";
        } else {
          popupField +=
            fieldValue != null
      ...
      if (layer.get("fieldAliases")[currentFeatureKeys[i]] == "Tipo E.S.") {
        popupText += `<tr><td colspan='2'>
          <a href='https://web-gis-laravel-production-df45.up.railway.app/establecimiento/${currentFeature.get(
            "CODESTSAL"
          )}/detalle' target="_blank">Ver detalle</a>
          </td></tr>`;
        popupText += `<tr><td colspan='2'>
          <a href='https://web-gis-laravel-production-df45.up.railway.app/establecimiento/${currentFeature.get(
            "CODESTSAL"
          )}/servicios' target="_blank">Ver servicios</a>
          </td></tr>`;
        popupText += `<tr><td colspan='2'>
          <a href='https://web-gis-laravel-production-df45.up.railway.app/establecimiento/${currentFeature.get(
            "CODESTSAL"
          )}/localidades' target="_blank">Ver localidades</a>
          </td></tr>`;
      }
    }
  }
  ...
  for (var i = 0; i < wms_layers.length; i++) {
    if (wms_layers[i][1] && wms_layers[i][0].getVisible()) {
      ...
      if (url) {
        ...
        var ldsRoller =
          '<div class="roller-switcher" style="height: 25px; width: 25px;"></div>';
